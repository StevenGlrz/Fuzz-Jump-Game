...
import com.fuzzjump.game.model.character.UnlockableBound;
...
public class Fuzzle extends Actor implements Appearance.AppearanceChangeListener {
    ...
    private Unlockable defaultFuzzleUnlockable = new Unlockable(FuzzJump.Game.getUnlockableDefinitions().getDefinition(0), 0, -1);
    private Unlockable defaultFrameUnlockable = new Unlockable(FuzzJump.Game.getUnlockableDefinitions().getDefinition(32), 1, -1);
    ...
    public Fuzzle(StageUI ui) {
        ...
        this.questionMark = ui.getTextureRegionDrawable("ui-question-mark");
    ...
    public Fuzzle(StageUI ui, Profile profile) {
        this(ui);
        setProfile(profile);
    }

    public Fuzzle(StageUI ui, Profile profile, boolean drawFrame) {
        this(ui, profile);
    ...
    public void setFuzzle(Unlockable fuzzle) {
        ...
        this.fuzzle = fuzzle;
        if (fuzzle != null) {
            fuzzleDrawable.setRegion(ui.getColored(fuzzle, false));
            sizeChanged();
        }
    }
    ...
    public void setFrame(Unlockable frame) {
        ...
        if (ui != null && frame != null)
            frameDrawable.setRegion(ui.getColored(frame, false));
    }
    ...
    public void setHead(Unlockable head) {
        ...
        if (ui != null && head != null) {
            headDrawable.setRegion(ui.getColored(head, false));
            updateBounds(headBounds, head.getDefinition(), headDrawable);
        }
    }
    ...
    public void setEyes(Unlockable eyes) {
        ...
        if (ui != null && eyes != null) {
            eyesDrawable.setRegion(ui.getColored(eyes, false));
            updateBounds(eyesBounds, eyes.getDefinition(), eyesDrawable);
        }
    }
    ...
    public void setFace(Unlockable face) {
        ...
        if (ui != null && face != null) {
            faceDrawable.setRegion(ui.getColored(face, false));
            updateBounds(faceBounds, face.getDefinition(), faceDrawable);
        }
    }
