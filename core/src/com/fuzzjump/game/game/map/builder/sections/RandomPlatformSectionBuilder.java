package com.fuzzjump.game.game.map.builder.sections;
...
import com.fuzzjump.game.game.map.builder.SectionBuilder;
...
public class RandomPlatformSectionBuilder extends SectionBuilder {
    ...
    public static final float MULTI_Y_COUNT = 0;
    ...
    @Override
    protected void platformGenerated(GamePlatform platform) {

    }

    @Override
    public void generateSection(float y, float height) {
        float rankRatio = rank / Profile.MAX_RANK;
        ...
        for(int i = 0; i < loops; i++) {
            ...
            for(int f = 0; f < count; f++) {
                ...
                GamePlatform platform = getPlatform(region, genX, genY, region.getRegionWidth(), region.getRegionHeight(), randomMapPlatform.getPhysicsY(), region.getRegionHeight());
                world.getPhysicsActors().add(platform);
                if(count == 1 && 10 > 5 + random.nextInt(10)) {
